...
{
	size_t wx; // # size in block x 
	size_t wy;

	//block-dir x corresponds to voxel direction j (data y-axis)
	//block-dir y corresponds to voxel direction k (data z-axis)


	//want wx*wy to be 32.  The setting below slightly optimizes thread sharing
	//if (ny > nz) {
	//	wx = 8; wy = 4;
	//}
	//else {
	...
	size_t warpsize = wx*wy; //should be wx*wy = multple or factor of 32
...
pbhe_gpu_algorithm_session<pbhe_t> * New_PBHE_GPUmem(int nt, int nx, int ny, int nz)
{
	int targetDevice = 0;
	...
	pbhe_gpu_algorithm_session<pbhe_t> * session = pbhe_gpu_algorithm_session < pbhe_t >::GetInstance() ;

	size_t typeSize = sizeof(pbhe_t);

	checkCudaErrors(cudaMalloc((void **)&(session->temp3D_tf_dev), nx *ny*nz* typeSize));
	checkCudaErrors(cudaMalloc((void **)&(session->temp3D_ti_dev), nx *ny*nz * typeSize));
	checkCudaErrors(cudaMalloc((void **)&(session->tdot3D_dev), nx *ny*nz * typeSize));
	checkCudaErrors(cudaMalloc((void **)&(session->kt3D_dev), nx *ny*nz * typeSize));
	checkCudaErrors(cudaMalloc((void **)&(session->rhoCp3D_dev), nx *ny*nz * typeSize));
	checkCudaErrors(cudaMalloc((void **)&(session->Dtxyz_d), 4 * typeSize));
...
int Pennes_2ndOrder_template(pbhe_t * temp4D, pbhe_t * tdot3D, pbhe_t * kt3D, pbhe_t * rhoCp3D,
	...
	int nt,
	int nx, int ny, int nz, int bcMode = FD_FREEFLOW_BOUNDARYCOND, pbhe_gpu_algorithm_session<pbhe_t> * session=NULL)
{
	...
	if (session == NULL)
	{
		...
		temp3D_tf_dev = session->temp3D_tf_dev;
		temp3D_ti_dev = session->temp3D_ti_dev;
		tdot3D_dev = session->tdot3D_dev;
		kt3D_dev = session->kt3D_dev;
		rhoCp3D_dev = session->rhoCp3D_dev;
		Dtxyz_d = session->Dtxyz_d;

		if (session->initlevel == 0)
		{
			checkCudaErrors(cudaMemcpy(kt3D_dev, kt3D, nx *ny*nz*  typeSize, cudaMemcpyHostToDevice));
			checkCudaErrors(cudaMemcpy(rhoCp3D_dev, rhoCp3D, nx *ny*nz* typeSize, cudaMemcpyHostToDevice));
			checkCudaErrors(cudaMemcpy(Dtxyz_d, Dtxyz, 4 * typeSize, cudaMemcpyHostToDevice));
...
int Pennes_2ndOrder_GPU(double * temp4D, double * tdot3D, double * kt3D, double * rhoCp3D,
	...
	int nt,
	int nx, int ny, int nz, int bcMode, pbhe_gpu_algorithm_session<double> * session )
{
	return Pennes_2ndOrder_template<double>(temp4D, tdot3D, kt3D, rhoCp3D, Dtxyz, Tblood, perfRate, nt, nx, ny, nz, bcMode, session);
}
...
int Pennes_2ndOrder_GPU_f(float * temp4D, float * tdot3D, float * kt3D, float * rhoCp3D,
	...
	int nt,
	int nx, int ny, int nz, int bcMode, pbhe_gpu_algorithm_session<float> * session)
{
	return Pennes_2ndOrder_template<float>(temp4D, tdot3D, kt3D, rhoCp3D, Dtxyz, Tblood, perfRate, nt, nx, ny, nz, bcMode, session);
}