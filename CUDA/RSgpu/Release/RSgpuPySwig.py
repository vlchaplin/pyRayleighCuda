...
if version_info >= (2, 6, 0):
    def swig_import_helper():
        ...
        fp = None
        try:
            fp, pathname, description = imp.find_module('_RSgpuPySwig', [dirname(__file__)])
        except ImportError:
            import _RSgpuPySwig
            return _RSgpuPySwig
        if fp is not None:
            try:
                _mod = imp.load_module('_RSgpuPySwig', fp, pathname, description)
            finally:
                fp.close()
            return _mod
    _RSgpuPySwig = swig_import_helper()
