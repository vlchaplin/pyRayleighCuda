//PBHE_FDsolve_mex( T ,  rhoCp,  kt, resolution )
		 ...
		 )
{
  ...
  if (nrhs != 8 && nrhs != 7) {
    mexErrMsgIdAndTxt("MATLAB:PBHE_Perfused_mex:nargin", 
            "7 or 8 arguments required: (T, Tdot_source, rhoCp, kt, resolution, bloodTemp, perfusionRate, [freeOutflowFlag])");
  } 
  ...
  if (ndims != 4) {
    mexErrMsgIdAndTxt("MATLAB:PBHE_Perfused_mex:ndims", 
  ...
  double bloodTemp = mxGetScalar( prhs[5] );
  double perfusionCoef = mxGetScalar( prhs[6] );

  if (nrhs==8)
    freeOut = (bool)mxGetScalar( prhs[7] );
  else
    freeOut = 0;
  ...
  for (t=0;t<nt-1;t++){
        ...
        for (i=1;i<nx-1;i++)
        {
            ...
            for (j=1;j<ny-1;j++)
            {
                ...
                for (k=1;k<nz-1;k++)
                {
                    ...
                    Pennes_Perfused(txyz, meshResolution, &T, tsdotptr, &Kt, &rCp, bloodTemp, perfusionCoef );
                }
            }
        }
        ...
        //  and heat would dissipate to zero at infinity. 
            ...
            for (i=0;i<nx;i++)
            {
                ...
                for (k=0;k<nz;k++)
                {
                    T(t+1,i,0,k) = T(t,i,0,k);
                    T(t+1,i,ny-1,k) = T(t,i,ny-1,k);
                }
                //X-Y boundaries
                for (j=0;j<ny;j++)
                {
                    T(t+1,i,j,0) = T(t,i,j,0);
                    T(t+1,i,j,nz-1) = T(t,i,j,nz-1);
                }
            }
            ...
            for (j=0;j<ny;j++)
            {
                for (k=0;k<nz;k++)
                {
                    T(t+1,0,j,k) = T(t,0,j,k);
                    T(t+1,nx-1,j,k) = T(t,nx-1,j,k);
                }
            }
